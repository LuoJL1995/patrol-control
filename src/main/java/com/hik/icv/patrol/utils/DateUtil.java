...
/**
 * @Description 日期工具类，统一采用JodaTime
 * @Author LuoJiaLei
 * @Date 2020/4/26
 * @Time 15:13
 */
public class DateUtil {
    ...
    public static final String HOUR_MINITER = "HH:mm";
     ...
     * @return
     */
    public static Date getDateAfter(Date date, int hour, int minutes, int seconds) {
        return new DateTime(date).plusHours(hour).plusMinutes(minutes).plusSeconds(seconds).toDate();
    }
     ...
     * @return
     */
    public static Date getEndOfDayWithTime(Date date, int offset) {
    ...
    public static Date plusTime(Date date, String time) {
        ...
        int miniter = Integer.valueOf(timeArr[0]);
        return DateUtil.getDateAfter(date, hour, miniter, 0);
    }

    /**
     * 根据开始时间 结束时间 当前时间换算百分比的整数值
     * （当前时间 - 开始时间）/ （结束时间-开始时间）
     */
    public static int getPercentInt(Date starTime, Date endTime, Date now) {
        //防止异常
        if (starTime == null || endTime == null || now == null){
            return 0;
        }
        //还没开始
        ...
        return (int)((now.getTime() - starTime.getTime())/(endTime.getTime() - starTime.getTime()));
    }
